...
static void	side_step(t_game *game, double raydir_x, double raydir_y)
{
	...
	{
		game->ray.step_x = -1;
		game->player.side_distx = (game->player.pos_x - game->ray.map_x) \
		* game->ray.delta_distx;
	}
	else
	{
		game->ray.step_x = 1;
		game->player.side_distx = (game->ray.map_x + 1.0 - game->player.pos_x) \
		* game->ray.delta_distx;
	}
	if (raydir_y < 0)
	{
		game->ray.step_y = -1;
		game->player.side_disty = (game->player.pos_y - game->ray.map_y) \
		* game->ray.delta_disty;
	}
	else
	{
		game->ray.step_y = 1;
		game->player.side_disty = (game->ray.map_y + 1.0 - game->player.pos_y) \
		* game->ray.delta_disty;
	}
}
...
static void	choose_textr(t_game *game, double raydir_x, double raydir_y, int x)
{
	if (game->player.side == 0)
		game->player.to_wall = (game->ray.map_x - game->player.pos_x + \
		(1 - game->ray.step_x) / 2) / raydir_x;
	else
		game->player.to_wall = (game->ray.map_y - game->player.pos_y + \
		(1 - game->ray.step_y) / 2) / raydir_y;
	...
	if (game->player.side == 1 && game->ray.step_y < 0)
		game->txtr.cur = game->txtr.wall_n;
	else if (game->player.side == 1 && game->ray.step_y > 0)
		game->txtr.cur = game->txtr.wall_s;
	else if (game->player.side == 0 && game->ray.step_x < 0)
		game->txtr.cur = game->txtr.wall_e;
	else if (game->player.side == 0 && game->ray.step_x > 0)
		game->txtr.cur = game->txtr.wall_w;
	if (game->map.map[game->ray.map_y][game->ray.map_x] == '2')
		game->txtr.cur = game->txtr.sprite;
}
...
double raydir_x, double raydir_y)
{
	...
	if (game->player.side == 0)
		game->txtr.wall_x = game->player.pos_y + game->player.to_wall \
		* raydir_y;
	else
		game->txtr.wall_x = game->player.pos_x + game->player.to_wall \
		* raydir_x;
	...
	while (game->ray.draw_start < game->ray.draw_end)
	{
		...
		game->txtr.cl = pxl(game->txtr.cur, game->txtr.tex_x, game->txtr.tex_y);
		my_mlx_pixel_put(game->data, x, game->ray.draw_start, game->txtr.cl);
		game->ray.draw_start++;
	}
}
